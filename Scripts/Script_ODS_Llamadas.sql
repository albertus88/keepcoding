...
SELECT *
FROM ODS.ODS_DM_DDEPARTAMENTOS_CC;
...
SELECT DISTINCT ID ID_IVR
, PHONE_NUMBER TELEFONO_LLAMADA
, -1
...
, FLG_TRANSFER FLG_TRANSFERIDO
, AGE.ID_AGENTE_CC ID_AGENTE_CC
, NOW()
, NOW()
FROM STAGE.STG_CONTACTOS_IVR CONTACTOS
INNER JOIN ODS.ODS_DM_AGENTES_CC AGE ON CASE WHEN TRIM(SERVICE)<>'' THEN UPPER(TRIM(CONTACTOS.SERVICE)) ELSE 'DESCONOCIDO' END=AGE.DE_AGENTE_CC
INNER JOIN ODS.ODS_DM_DEPARTAMENTOS_CC DEPT ON CASE WHEN TRIM(AGENT)<>'' THEN UPPER(TRIM(CONTACTOS.AGENT)) ELSE 'DESCONOCIDO' END=DEPT.DE_DEPARTAMENTO_CC;
